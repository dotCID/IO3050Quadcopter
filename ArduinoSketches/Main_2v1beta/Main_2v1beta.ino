...
#define LIFTOFF_SPEED 178
#define FLY_SPEED 184
...
double Kp_x	= 0.0250;			// stable single axis: 0.025
double Ki_x	= 0.001;			// ssa: 0.001
double Kd_x	= 0.0015;				// ssa:0.0001
double pid_x_neg_target = 0.0;	// 0 because neutralPoint should compensate

double Kp_y	= 0.0150;			// ssa: 0.014
double Ki_y	= 0.001;			// ssa: 0.001
double Kd_y	= 0.015;			// ssa: 0.001
	...
	if(debugMode && Serial.available() > 0){
		...
		if (ch != 10){
			...
			if(incomingString.startsWith("Kp_x = ")){
				...
				if(val==-1){
				   Serial.println("Killing all engines");
				   killed = true;
				   write(1,20);
				   write(2,20);
				   write(3,20);
